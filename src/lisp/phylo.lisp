...
;; * start the server
;; * set *default-ip* and *default-port*
;; * (everything) to set up the population
;; * (dispatch-section ".text" :reset_data 1 :ip <ip address> :port <port>)
;; * set the target pattern with (init-target) or edit (everything)
;; * to start evaluating chains:
;; * (test-chain (pick *population*))
;; * to run tournements and do the evolution:
;;   (tournement 4 *population* :ip <ip> :port <port>)
;; * repeat, or put this in a loop.
;; * to speed up, turn off the DEBUG macro flag in listener.c
;; =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
